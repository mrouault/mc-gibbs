...
sample_mala_reshaped = sample_gibbs["samples"].reshape((n_iter, d, n))
...
for t in range(n_iter):
    if t % 25 == 0:
        output_dir = "animation_gibbs"
        filename = os.path.join(output_dir, f"frame_{frame_id:03d}.pdf")
        frame_id += 1

        print(f"Iteration {t}/{n_iter}")

        fig = plt.figure(figsize=(5, 8))
        axes = fig.add_axes([0, 0, 1, 1])  # axes fill the figure
        #axes.set_xlim(-2, 2)
        #axes.set_ylim(-2, 2)
        axes.scatter(*sample_mala_reshaped[t, :, :], alpha=0.8, s=10)

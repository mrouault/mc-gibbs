...
#step size is 5e-5 for beta_n = n**2 and beta_n = n**3
key_mcmc = 0
step_size_mcmc_env = 1e-1
step_size_gibbs = 5e-5
n_iter_env = 1_000
n_iter_gibbs = 10_000
n = 100
beta_n = n**2
...
for k in range(5):
    ...
    with open("run_xp_gibbs_"+s+".slurm", "w") as file:
        ...
        file.write("python3 samples_coverage_gibbs.py --key_mcmc="+str(key_mcmc)+" --key_gibbs="+str(key_gibbs)+" --step_size_mcmc_env="+str(step_size_mcmc_env)+" --step_size_gibbs="+str(step_size_gibbs)+" --n_iter_env="+str(n_iter_env)+" --n_iter_gibbs="+str(n_iter_gibbs)+" --n="+str(n)+" --beta_n="+str(beta_n)+" \n")
